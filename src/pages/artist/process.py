...
csvfile = open('data.csv', 'r', encoding="utf8")
...
for row_index, row in enumerate(datareader):
	...
	# Othrwise, create a YAML file from the data in this row...
		...
		filename = row[2].lower().replace(" ", "-") + '.md'
		new_yaml = open(filename, 'w', encoding="utf8")

		# Empty string that we will fill with YAML formatted text based on data extracted from our CSV.
		yaml_text = ""
		yaml_text += "---\n"
		yaml_text += "templateKey: artist-post\ntitle: Exhibition Title\nfeaturedimage: /img/artist-thumbs/"+row[2].lower().replace(" ", "-")+".png\n"
		...
		for cell_index, cell in enumerate(row):
			...
			cell_text = ""
			if cell_heading == 'statement' or cell_heading == 'bio':
