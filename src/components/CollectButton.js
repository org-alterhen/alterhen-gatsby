...
const CollectButton = function ({ objkt, contract, userAddress }) {
  ...
  return (
      <>
        ...
        {objkt.hicdex.swaps && objkt.hicdex.swaps[objkt.hicdex.swaps.length - 1]?.amount_left > 0 ? (
            <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const op = await contract.methods
                        .collect(objkt.hicdex.swaps[objkt.hicdex.swaps.length - 1].id)
                        .send({
                          amount: objkt.hicdex.swaps[objkt.hicdex.swaps.length - 1].price, // parseFloat(objkt.price)
                          mutez: true,
                          storageLimit: 310,
                        })
            ...
            >
              Collect for {objkt.hicdex.swaps[objkt.hicdex.swaps.length - 1].price / 1000000} tez
            </button>
        ) : (
            <a
